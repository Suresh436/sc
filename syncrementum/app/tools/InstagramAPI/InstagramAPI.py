...
class InstagramAPI:
    ...
    def twoFactorLogin(self, verification_code='', two_factor_identifier=''):
        if (not self.isLoggedIn or force):
            if (self.SendRequest('si/fetch_headers/?challenge_type=signup&guid=' + self.generateUUID(False), None, True)):

                data = {' ': self.LastResponse.cookies['csrftoken'],
                        'username': self.username,
                        'guid': self.uuid,
                        'device_id': self.device_id,
                        'password': self.password,
    ...
    def sendChallengeCode(self, choice=1, apipath=None):
        if (not self.isLoggedIn or force):
            if (self.SendRequest('si/fetch_headers/?challenge_type=signup&guid=' + self.generateUUID(False), None, True)):
                ...
                if (self.SendRequest(apipath, self.generateSignature(json.dumps(data)), True)):
                    self.token = self.LastResponse.cookies["csrftoken"]
                    print("Sent success!\n")
                    return True

    def LoginWithSecurityCode(self, security_code='', apipath=None):
        if (not self.isLoggedIn or force):
            if (self.SendRequest('si/fetch_headers/?challenge_type=signup&guid=' + self.generateUUID(False), None, True)):
                ...
                if (
                self.SendRequest(apipath, self.generateSignature(json.dumps(data)),
                                 True)):
